...
const AllInnerCards = document.querySelectorAll(".slider-cards");
...
const moveRight = () => {
    ...
    if (availableRight > 0) {
        ...
        if (availableLeft == 0 && noOfCards >= availableRight) {
            setRight = (availableRight * innerWidth) + 32 - (mainWidth - (noOfCards * innerWidth) - 32);
            availableLeft += availableRight;
			availableRight = 0;
			rightArrow.style.display = "none";
        }
        else if (availableLeft == 0) {
            setRight = noOfCards * innerWidth + 32 - ((mainWidth - noOfCards * innerWidth) / 2);
            availableLeft += noOfCards;
            availableRight -= noOfCards;
        }
        else if (noOfCards >= availableRight) {
            setRight = (availableRight * innerWidth) + ((mainWidth - (noOfCards * innerWidth)) / 2) - mainWidth + (noOfCards * innerWidth) + 32;
...
const moveLeft = () => {
    ...
    if (availableLeft > 0) {
        ...
        if (availableRight == 0) {
            setLeft = (noOfCards * innerWidth) + 32 - ((mainWidth - noOfCards * innerWidth) / 2);
            availableLeft -= noOfCards;
            availableRight += noOfCards;
        }
